...
fun DesignSystemPlaygroundScreen() {
    ...
    val components: List<Pair<String, @Composable () -> Unit>> = listOf(
        "Toolbar" to { BinaSearchToolbarPreviewContent() },
        "List Item" to { CharacterListItemPreviewContent() },
        "Dialog" to { BinaErrorDialogPreviewContent() }
    )

    BinaAppTheme(darkTheme = isDarkTheme) {
        Scaffold(
            topBar = {
                TopAppBar(
            ...
            Column(
                ...
                components.firstOrNull { it.first == selectedComponent }?.second?.invoke()
                    ?: Text("Componente não encontrado", color = Color.Red)
            }
        }
    }
}
